...
async function drawGift() {
    ...
    if (availableParticipants.length === 0) {
        // DEADLOCK KONTROLÜ: Eğer sona kalan kişi kendisi ise (Deadlock)
        const unassignedParticipants = participants.filter(p => !alreadyAssigned.includes(p.name));
        
        if (unassignedParticipants.length === 1 && unassignedParticipants[0].name === currentUser) {
            // Deadlock durumu tespit edildi! Swap işlemi uygulanacak.
            // Rastgele bir kura seç (Kurban)
            const randomDrawIndex = Math.floor(Math.random() * draws.length);
            const victimDraw = draws[randomDrawIndex];
            
            // Kurbanın hediyesi artık mevcut kullanıcıya gidecek
            // Mevcut kullanıcı ise kurbanın eski hediyesini alacak
            const originalRecipientName = victimDraw.recipient;
            
            // 1. Kurbanın kurasını güncelle (Kurban -> CurrentUser)
            const updateSuccess = await updateDraw(victimDraw.id, { recipient: currentUser });
            
            if (!updateSuccess) {
                showMessage(messageEl, 'Sistem hatası (Swap Update)!', 'error');
                return;
            }
            
            // 2. Mevcut kullanıcının kurasını oluştur (CurrentUser -> OriginalRecipient)
             const drawResult = {
                userName: currentUser,
                selectedNumber: selectedNumber,
                recipient: originalRecipientName,
            ...
            if (success) {
                ...
                setTimeout(() => {
                    showResult(originalRecipientName);
                }, 1000);
            } else {
